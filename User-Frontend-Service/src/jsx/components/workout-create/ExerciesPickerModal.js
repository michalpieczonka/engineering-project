...
import {  addExercise } from '../../../store/actions/WorkoutWizardActions'
import { mapTrainingDay, mapMuscleGroup } from './CommonMappers';


export default function ExerciesPickerModal({muscleGroup, disableModal}) {
   const dispatch = useDispatch();
   const AC = bindActionCreators(    { addExercise  },    dispatch)
   console.log(AC)

  const planDetails = useSelector((state) => state.workoutwizard)
      ...
      if (numberOfSeries > 10){
         ...
         if(numberOfSeries !== 0 && index === -1 && event === -1){
            ...
            for(let i=0; i<numberOfSeries; i++){
               tmpUnitDetails.push( { numberOfRepetitions: '' })
            }
            setTrainingUnitDetails(tmpUnitDetails)
   
         } else if(numberOfSeries === -1){
            let data = [...trainingUnitDetails];
            data[index].numberOfRepetitions = event.target.value;
            setTrainingUnitDetails(data);
            console.log(data)
         }
      }
...
const handleChangeNewExerciseAdditionalMuscles = (e) => {
  ...
  const trainingDaysOptions = planDetails.trainingDays.map(it => ({value: it, label: mapTrainingDay(it)}))
 ...
 >
    ...
    <Modal.Body>
      ...
      <h4 class="mm-b-0">Dodawanie ćwiczenia do planu treningowego</h4>
